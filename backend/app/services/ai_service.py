...
def generate_financial_analysis(company_financials: Dict[str, pd.DataFrame]) -> str | None:
    ...
    try:
        ...
        for statement, df in company_financials.items():
            # Selecionar apenas as colunas mais relevantes para a análise
            # Estamos interessados na descrição da conta e no seu valor.
            # Também filtramos para pegar a versão mais recente dos dados (último trimestre)
            # e a última versão do relatório (VERSAO).
            df_filtered = df.loc[df.groupby('CD_CONTA')['VERSAO'].idxmax()] # Pega a última versão de cada conta
            df_filtered = df_filtered.loc[df_filtered.groupby('CD_CONTA')['DT_FIM_EXERC'].idxmax()] # Pega o último período
            
            # Selecionando colunas relevantes e removendo duplicatas
        ...
        Baseado nos dados financeiros fornecidos em formato JSON, gere uma análise que cubra os seguintes pontos:
        ...
        user_prompt_content = f"Por favor, analise os seguintes dados financeiros: \n{json.dumps(financial_data_json, indent=2, ensure_ascii=False)}"
